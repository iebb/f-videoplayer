...
- (instancetype)initWithRegistrar:(NSObject<FlutterPluginRegistrar>*)registrar {
    ...
    _pictureInPictureChannel = [FlutterMethodChannel
        methodChannelWithName:@"f_videoplayer/fvp_picture_in_picture"
...
- (BOOL)preparePictureInPicture:(id)arguments {
    if (@available(iOS 15.0, *)) {
        ...
        if (playerIterator == players.end()) {
            NSLog(@"F Video Player FVP PiP prepare failed: player %lld not found", textureId);
            return NO;
        }
        ...
        NSLog(@"F Video Player FVP PiP prepared active player %lld", textureId);
        return YES;
    }
    return NO;
}
    ...
    if (@available(iOS 15.0, *)) {
        ...
        if (self.pictureInPictureController.isPictureInPicturePossible) {
            if (!self.pictureInPictureStartRequested) {
                ...
                NSLog(@"F Video Player FVP PiP startPictureInPicture");
        ...
        if (attempts <= 0) {
            NSLog(@"F Video Player FVP PiP start timed out waiting for possible");
...
- (void)pictureInPictureController:
    ...
    NSLog(@"F Video Player FVP PiP failed to start: %@", error.localizedDescription);
    FlutterResult result = self.pendingPictureInPictureResult;
    self.pendingPictureInPictureResult = nil;
    if (result)
        result(@NO);
