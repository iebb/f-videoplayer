...
/// Included iOS implementation of F Video Player's shared system-PiP channel.
  ...
  AVPictureInPictureControllerDelegate
{
  ...
  public static func register(with registrar: FlutterPluginRegistrar) {
    let instance = FVideoPictureInPicturePlugin(messenger: registrar.messenger())
  ...
  private static func rootViewController() -> UIViewController? {
    if #available(iOS 13.0, *) {
      ...
      return topViewController(
        from: scene?.windows.first { $0.isKeyWindow }?.rootViewController
      )
    }
    return topViewController(from: UIApplication.shared.keyWindow?.rootViewController)
  }
