# F Video Player example

This is a buildable Android, iOS, Linux, macOS, Windows, and web catalog for
`f_videoplayer`. It intentionally uses only Flutter's foundational
...
The example adds the optional `f_videoplayer_fvp` adapter and calls
`FVideoFvpBackend.ensureInitialized()` before any video controller is created.
...
avoiding both duplicate and undefined Flutter symbols in version 0.5.0.
...
`../../../third_party/fvp` (`0.37.3+fvideo.8`). Overrides from a dependency are
...
```sh
flutter run -d linux \
  --dart-define=F_VIDEO_FILE=/absolute/path/to/video.mp4
```
...
```sh
flutter run -d macos \
  --dart-define=F_VIDEO_ASSET=assets/sample.mp4
```
...
through `FVideoPlayer.controller`, and disposes it from the host widget's
...
double-taps. The player uses `FVideoInteractionMode.delegateToChrome`, so
...
passes every process entry through `FVideoDesktopWindows.initialize`, and
the child entry point renders inside `FVideoDesktopWindowHost`. On Linux,
