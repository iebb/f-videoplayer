...
class _ExampleHomeState extends State<_ExampleHome> {
  ...
  void initState() {
    super.initState();
    unawaited(_refreshPictureInPictureSupport());
  }
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      _PlayerFooter(
                        ...
                        windowEnabled:
                            fVideoSupportsDesktopVideoWindows &&
  ...
  Widget _buildPlayer() => FVideoPlayer(
    key: _playerKey,
    ...
    onPictureInPictureChanged:
        _pictureInPictureSupported && _pictureInPictureUri != null
  ...
  Uri? get _pictureInPictureUri {
    final source = sourceForExampleMode(_mode);
    return switch (source.kind) {
      FVideoSourceKind.network => Uri.tryParse(source.location),
      FVideoSourceKind.file => Uri.file(source.location),
      FVideoSourceKind.asset => null,
    };
  }

  Future<void> _refreshPictureInPictureSupport() async {
    final supported = await FVideoPictureInPicture.isSupported();
    if (mounted) setState(() => _pictureInPictureSupported = supported);
  }

  Future<void> _setPictureInPicture(bool enabled) async {
    if (!enabled) {
      await FVideoPictureInPicture.stop();
      if (mounted) setState(() => _pictureInPicture = false);
      return;
    }
    final controller = _activeController;
    final uri = _pictureInPictureUri;
    if (controller == null || uri == null || !controller.value.isInitialized) {
      return;
    }
    ...
    final started = await FVideoPictureInPicture.start(
      ...
      videoSize: value.size.isEmpty ? const Size(16, 9) : value.size,
      playerId: controller.fvpPlayerId,
      ...
      onRestoreRequested: (snapshot) async {
        await _restorePictureInPictureSnapshot(controller, snapshot);
        if (mounted) setState(() => _pictureInPicture = false);
        return true;
      },
    ...
    if (!FVideoPictureInPicture.keepsFlutterPlayerInActivity &&
        !FVideoPictureInPicture.usesActivePlayer(_pipSessionId)) {
      await controller.pause();
    }
    if (mounted) {
      setState(() {
        _pictureInPicture = true;
        _status = 'Playing in system Picture in Picture';
      });
    }
  }

  Future<void> _restorePictureInPictureSnapshot(
  ...
  Future<void> _openWindow() async {
    if (!fVideoSupportsDesktopVideoWindows) return;
    final uri = _mode == ExampleSourceMode.file
        ? Uri.file(configuredFileVideo)
        : Uri.parse(sampleVideo);
    final id = await FVideoDesktopWindows.instance.open(
...
class ExampleWindow extends StatelessWidget {
  ...
  final FVideoDesktopWindowArguments arguments;
