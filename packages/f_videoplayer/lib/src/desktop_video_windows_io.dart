...
class _IoDesktopVideoWindows
    with WindowListener
    implements FVideoDesktopWindowsPlatform {
  final Map<int, FVideoDesktopWindowClosed> _closeCallbacks = {};
  ...
  Future<FVideoDesktopWindowArguments?> initialize(
    ...
    return FVideoDesktopWindowArguments.tryParse(
      arguments[1],
          ...
          await controller
              .waitUntilReadyToShow(
                WindowOptions(
                  ...
                  center: true,
                  title: FVideoDesktopWindowArguments.normalizeTitle(
                    arguments.title,
                  ),
                ),
              )
  ...
  Future<void> configureCurrentWindow({
    ...
    try {
      ...
      await window.setTitle(FVideoDesktopWindowArguments.normalizeTitle(title));
      await window.center();
    } on Object catch (error, stackTrace) {
  ...
  Future<void> _runCloseCallback(FVideoDesktopWindowClosed? callback) async {
  ...
  void _report(String operation, Object cause, StackTrace stackTrace) {
    final error = FVideoDesktopWindowException(operation, cause);
