...
const fVideoDesktopWindowType = 'f_videoplayer.video';
const _windowProtocolVersion = 1;

/// Serializable startup data passed to an independent video window.
@immutable
class FVideoDesktopWindowArguments {
  const FVideoDesktopWindowArguments({
    required this.uri,
    required this.title,
    required this.width,
    required this.height,
    required this.muted,
    this.windowType = fVideoDesktopWindowType,
  });
  ...
  static FVideoDesktopWindowArguments? tryParse(
    String source, {
    String windowType = fVideoDesktopWindowType,
  }) {
    ...
    try {
      ...
      return FVideoDesktopWindowArguments(
        uri: uri,
...
typedef FVideoDesktopWindowClosed = FutureOr<void> Function();
typedef FVideoDesktopWindowErrorHandler =
    void Function(FVideoDesktopWindowException error, StackTrace stackTrace);

/// A non-fatal error raised by the native desktop-window integration.
@immutable
class FVideoDesktopWindowException implements Exception {
  const FVideoDesktopWindowException(this.operation, this.cause);

  final String operation;
  final Object cause;

  @override
  String toString() => 'FVideoDesktopWindowException($operation): $cause';
}
