...
import 'package:f_videoplayer/src/desktop_video_windows.dart';
import 'package:f_videoplayer/src/desktop_video_windows_stub.dart' as stub;

void main() {
  TestWidgetsFlutterBinding.ensureInitialized();

  test('arguments accept legacy payloads and sanitize optional metadata', () {
    final legacy = FVideoDesktopWindowArguments.tryParse(
      '{"type":"f_videoplayer.video","uri":"https://example.test/video.mp4",'
    ...
    expect(
      FVideoDesktopWindowArguments.tryParse(
        '{"version":2,"type":"f_videoplayer.video",'
  ...
  test(
    ...
    () async {
      ...
      expect(await FVideoDesktopWindows.initialize(const []), isNull);
      var firstClosed = 0;
      var secondClosed = 0;
      final ids = await Future.wait([
        FVideoDesktopWindows.instance.open(
          _arguments('First'),
      ...
      expect(ids, [40, 41]);
      expect(FVideoDesktopWindows.instance.activeWindowIds, {40, 41});
      ...
      expect(FVideoDesktopWindows.instance.activeWindowIds, {41});
      ...
      expect(FVideoDesktopWindows.instance.activeWindowIds, isEmpty);
    },
  );
  ...
  test(
    ...
    () async {
      ...
      final messenger =
          TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger;
      final windowCalls = <MethodCall>[];
      FVideoDesktopWindows.instance.onError = (_, _) {};
      addTearDown(() => FVideoDesktopWindows.instance.onError = null);
...
FVideoDesktopWindowArguments _arguments(String title) =>
    FVideoDesktopWindowArguments(
      uri: Uri.parse('https://example.test/video.mp4'),
      title: title,
      width: 1920,
