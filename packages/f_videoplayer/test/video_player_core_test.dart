...
void main() {
  testWidgets('caller-owned controller follows lifecycle and is not disposed', (
    ...
    await tester.pumpWidget(
      _testHost(
        FVideoPlayer(
          source: const FVideoSource.asset('unused.mp4'),
  ...
  testWidgets('source changes do not reinitialize a supplied controller', (
    ...
    await tester.pumpWidget(
      _testHost(
        FVideoPlayer(
          source: const FVideoSource.network('https://one.invalid/a.mp4'),
          controller: controller,
          autoplay: false,
    ...
    await tester.pumpWidget(
      _testHost(
        FVideoPlayer(
          source: const FVideoSource.network('https://two.invalid/b.mp4'),
          controller: controller,
          autoplay: false,
          onReady: (_) => readyCalls++,
        ),
  ...
  testWidgets('stale async controller creation cannot win a source race', (
    ...
    FutureOr<VideoPlayerController> builder(FVideoSource source) =>
        source.location.contains('first') ? firstCompleter.future : second;

    await tester.pumpWidget(
      _testHost(
        FVideoPlayer(
          source: const FVideoSource.network('https://media.invalid/first.mp4'),
  ...
  testWidgets('controller creation errors expose safe retry UI', (
    ...
    FVideoPlayerError? reported;
    await tester.pumpWidget(
      _testHost(
        FVideoPlayer(
          source: const FVideoSource.asset('missing.mp4'),
  ...
  testWidgets('invalid zero seek interval fails before controller creation', (
    ...
    await tester.pumpWidget(
      _testHost(
        const FVideoPlayer(
          source: FVideoSource.asset('unused.mp4'),
