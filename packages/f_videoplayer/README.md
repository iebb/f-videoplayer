...
`f_videoplayer` is a reusable, responsive Flutter player surface built on
...
This repository validates `third_party/fvp` version `0.37.3+fvideo.6`, which
...
The `0.37.3+fvideo.6` fork also fixes the macOS duplicate-Flutter-framework
...
Future<void> main(List<String> arguments) async {
  ...
  final child = await FVideoDesktopWindows.initialize(arguments);
...
`FVideoFvpConfiguration` to set decoder preferences, decoded-size bounds,
...
artifacts. See the
[`f_videoplayer_fvp` README](../f_videoplayer_fvp/README.md) for the
...
```dart
FVideoPlayer(
  source: const FVideoSource.network(
...
```dart
const network = FVideoSource.network(
  'https://media.example/video.mp4',
  httpHeaders: {'X-Playback-Token': '...'},
);

const asset = FVideoSource.asset(
  'videos/intro.mp4',
  package: 'feature_assets', // Omit for an app-owned asset.
);

const file = FVideoSource.file('/absolute/path/to/video.mp4');

final fromUri = FVideoSource.uri(uri);
```

`FVideoSource.network` also accepts the official controller options and a
...
`FVideoChromeScope.previous` and `.next`.

### Default chrome styling and top-trailing actions

`FVideoChromeStyle` configures the dependency-free default chrome without
...
Pass `FVideoPlayerLabels` for localized control, state, timeline, error,
...
- `FVideoLifecycleBehavior.pauseAndResume` for a dedicated playback
  screen;
- `FVideoLifecycleBehavior.pause` without automatic resume for feeds and
  chat attachments; or
- `FVideoLifecycleBehavior.delegateToController` when a higher-level media
...
At process startup, call `FVideoDesktopWindows.initialize(arguments)` and
render a child player when it returns arguments. Wrap child content in
`FVideoDesktopWindowHost`; Linux retains and reuses hidden secondary
...
FVideoDesktopWindowHost(
  ...
  builder: (context, arguments) => ValueListenableBuilder<bool>(
    ...
    builder: (context, fullscreen, _) => FVideoPlayer(
      ...
      onClose: FVideoDesktopWindows.closeCurrentWindow,
      onFullscreenChanged: (value) {
        unawaited(
          FVideoDesktopWindows.setCurrentWindowFullscreen(value),
        );
      },
    ),
  ),
...
- The official `video_player` implementation does not include Windows/Linux;
  add `f_videoplayer_fvp` and call
  `FVideoFvpBackend.ensureInitialized()` before creating a controller there.
...
```sh
cd packages/f_videoplayer
flutter pub get
flutter analyze --fatal-infos
flutter test

cd ../f_videoplayer_fvp
flutter pub get
flutter analyze --fatal-infos
flutter test

cd ../f_videoplayer/example
flutter analyze --fatal-infos
flutter test
flutter build web --release
```

The path-filtered `F Video Player` workflow also compile-smokes the example
