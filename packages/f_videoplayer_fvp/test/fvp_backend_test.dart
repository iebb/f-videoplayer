...
void main() {
  test('defaults select only desktop platforms without official backends', () {
    const configuration = FVideoFvpConfiguration();
  ...
  test('typed options preserve advanced playback tuning', () {
    const configuration = FVideoFvpConfiguration(
      ...
      maxWidth: 2560,
      maxHeight: 1440,
      lowLatency: FVideoFvpLowLatency.live,
  ...
  test('decoded dimensions are validated at runtime when serialized', () {
    expect(
      () => const FVideoFvpConfiguration(maxWidth: 0).toFvpOptions(),
      throwsA(
        isA<ArgumentError>()
    ...
    expect(
      () => const FVideoFvpConfiguration(maxHeight: -1).toFvpOptions(),
      throwsA(
        isA<ArgumentError>()
  ...
  test('desktop child engines can decline process-global log ownership', () {
    const configuration = FVideoFvpConfiguration(
      platforms: {FVideoFvpPlatform.macos},
