...
enum FVideoFvpPlatform { android, ios, linux, macos, windows }

/// FVP's network playback latency policy.
enum FVideoFvpLowLatency {
...
class FVideoFvpConfiguration {
  const FVideoFvpConfiguration({
    this.platforms = const {FVideoFvpPlatform.linux, FVideoFvpPlatform.windows},
    this.fastSeek = false,
    this.videoDecoders = const [],
    this.maxWidth,
    this.maxHeight,
    this.lowLatency = FVideoFvpLowLatency.disabled,
  ...
  final Set<FVideoFvpPlatform> platforms;
  ...
  final FVideoFvpLowLatency lowLatency;
  ...
  Map<String, Object> toFvpOptions() {
    ...
    return <String, Object>{
      ...
      'maxWidth': ?maxWidth,
      'maxHeight': ?maxHeight,
      if (lowLatency != FVideoFvpLowLatency.disabled)
...
abstract final class FVideoFvpBackend {
  ...
  static void ensureInitialized({
    FVideoFvpConfiguration configuration = const FVideoFvpConfiguration(),
  }) {
  ...
  /// Routes only local WebM files through FVP on Android.
  ///
  /// Regular videos remain on the official platform backend, while `.webm`
  /// files in local file sources use FFmpeg so their VP9 alpha plane is
  /// preserved. Calls on other platforms are harmless no-ops.
  static void ensureAndroidAlphaWebmDecoderInitialized() {
    if (_initialized) return;
    backend.registerAndroidAlphaWebmDecoder();
    _initialized = true;
  }
}
